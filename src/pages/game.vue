<template>
<q-page :class="[gameReady && gameType ? 'ovh' : '']">
  <div class="container " >
      ...
      <p class="no-margin text-bold text-caption text-uppercase cursor-pointer" @click="resetGame">back to games</p>
		...
		<div v-if="!gameType && !gameReady" style="height: 100vh" class="tasks-content">
			<div class="container-div_h1">Tasks</div>
			<div class="tasks-content_block">
				<div class="mytasks">
					<div class="tasks-content_block-top">
						<div class="h1">My tasks for today</div>
						<div class="h2">28.05</div>
					</div>
					<div class="tasks-content_block-bottom">
						<div class="h1">My rating</div>
            <q-linear-progress rounded size="20px" :value="progress" color="warning" class="q-mt-sm" >
              <div class="absolute-full flex flex-center">
        <q-badge color="white" text-color="accent" :label="$user.user.rating" />
      </div>
            </q-linear-progress>
						<div class="date">17 days left until withdrawal</div>
					</div>
				</div>
				<div class="chose-block">
					<div class="chose-block_l">Choose a game</div>
<!--					<div v-if="can_play" class="chose-block_r">You have {{$user.user.games_count}} tasks left for today</div>-->
				</div>
				<div v-if="can_play" class="game-block">
					<div @click="selectGame(type)" class="game-block_select cursor-pointer" v-for="type in gameTypes" :key="type.id">
						<img class="game-block_image" :src="type.img" alt="QR PUZZLE">
						<div class="game-block_title">{{type.name}}</div>
					</div>
     ...
     <div style="height: 90vh" v-if="gameType && !gameReady" class="flex column items-center justify-center ">
      <p class="text-bold text-h4 text-uppercase q-mb-md">choose_difficult</p>
        ...
        <q-card
          ...
          <q-card-section  class="no-padding full-height">
            <div class="flex column items-center justify-evenly full-height">
              <p class="no-margin text-uppercase "><span class="text-weight-medium">level:</span> {{level.name}}</p>
              <p class="no-margin text-uppercase "><span class="text-weight-medium">pieces:</span> {{level.pieces}}</p>
              <p class="no-margin text-uppercase "><span class="text-weight-medium">rating:</span> +{{level.rating}}</p>
              <p class="no-margin text-uppercase "><span class="text-weight-medium">timer:</span> {{level.timer}} minutes</p>
            </div>
          </q-card-section>
        </q-card>
      </div>
    ...
    <div v-if="gameReady && gameType" style="position: relative" id="puzzle_wrapper" >
   ...
   <q-dialog v-model="showAd"
              ...
              @before-show="getAd"
              @before-hide="closeAd">
       <q-card class="bg-blue-8" style="width: 700px; max-width: 80vw;">
...
export default {
  ...
  data () {
    return {
      //slider
      progress:this.$user.user.rating *0.005,
      src: null,
      show: true,
       gameImg:null,
      videoTitle: 'Cat',
      ...
      AdVideo:[],
      gameTypes:[
        {id:1,name:'QR PUZZLE',type:'puzzle_qr',img:'qrpuzzle.svg',is_active:true,is_slider:false},
        {id:2,name:'ART PUZZLE',type:'puzzle_image',img:'artpuzzle.svg',is_active:true,is_slider:false},
        {id:3,name:'SLIDER PUZZLE',type:'new_puzzle',img:'sliderpuzzle.svg',is_active:false,is_slider:true},
      ],
    }
  },
	...
	&-content
		...
		.game-block
			...
			&_select
				...
				text-align: center
				&.active
					filter: drop-shadow(0px 4px 25px rgba(0, 102, 255, 0.25))
					border: 3px solid #4925AF
			&_title
				margin-top: 10px
				font-weight: 500
				font-size: 16px
